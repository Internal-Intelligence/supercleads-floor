...
function waitForPopupToken(popup: Window): Promise<string | null> {
  return new Promise((resolve, reject) => {
    ...
    const settle = (token: string | null, error?: string) => {
      ...
      settled = true;
      cleanup();
      if (!token && error) reject(new Error(error));
      else resolve(token);
    };
...
/** Email + password for any teammate. Stores the preview bearer when cookies are partitioned. */
export async function signInWithEmail(input: {
  ...
  if (!email || !email.includes("@")) throw new Error("Enter a real work email");
